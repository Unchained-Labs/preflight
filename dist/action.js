...
async function upsertComment(body) {
    ...
    try {
        ...
        if (!res.ok)
            log("warning", `could not post the comment: ${res.status} ${await res.text()}`);
        else
            log("notice", mine ? "updated the existing preflight comment" : "posted a preflight comment");
    }
    catch (e) {
