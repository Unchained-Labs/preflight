...
export function terminal(e) {
    const out = [];
    out.push("");
    out.push(`  ${bold(e.name ?? "workflow")}  ${grey("estimated before running")}`);
    out.push("");
    out.push(`  ${dim("agents")}     ${String(e.agents.expected).padStart(5)}  ${grey(`(${e.agents.low}–${e.agents.high})`)}`);
    out.push(`  ${dim("cost")}       ${bold(money(e.usd.expected)).padStart(5)}  ${grey(`(${money(e.usd.low)}–${money(e.usd.high)})`)}`);
    out.push(`  ${dim("tokens")}     ${compact(e.tokens.input).padStart(5)} in  ${compact(e.tokens.output)} out`);
    ...
    for (const [kind, usd] of Object.entries(e.byKind).sort((a, b) => b[1] - a[1])) {
        ...
        out.push(`  ${KIND_LABEL[kind].padEnd(10)} ${paint("█".repeat(width))}${grey("░".repeat(28 - width))} ${money(usd).padStart(8)}  ${grey(`${Math.round(share * 100)}%`)}`);
    }
    out.push("");
    if (e.byKind.verifier / total > 0.4) {
        out.push(`  ${yellow("!")} verification is ${Math.round((e.byKind.verifier / total) * 100)}% of this run. ` +
    ...
    out.push(`  ${grey("node".padEnd(w))}  ${grey("kind".padEnd(9))} ${grey("model".padEnd(18))} ${grey("calls".padStart(6))} ${grey("cost".padStart(9))}`);
    for (const n of e.nodes) {
        const assumed = n.widthAssumed ? yellow("~") : " ";
        out.push(`  ${n.id.padEnd(w)}  ${KIND_LABEL[n.kind].padEnd(9)} ${n.model.padEnd(18)} ${assumed}${String(n.calls.expected).padStart(5)} ${money(n.usd.expected).padStart(9)}`);
    }
    ...
    if (e.warnings.length) {
        for (const wn of e.warnings)
            out.push(`  ${yellow("!")} ${dim(wn)}`);
        out.push("");
    }
    out.push(grey(`  assumptions: ${e.assumptions.profiles.worker.input / 1000}k in / ${e.assumptions.profiles.worker.output} out per fan-out call, ` +
 ...
 */
export function markdown(d, opts = { file: "" }) {
    const e = d.after;
    const total = e.usd.expected || 1;
    const lines = [MARKER];
    const arrow = d.usdDelta === null
        ? ""
        : d.usdDelta > 0.005
            ? `📈 **+${money(d.usdDelta)}**`
            : d.usdDelta < -0.005
    ...
    lines.push("");
    if (arrow) {
        lines.push(`${arrow} against the base branch${d.agentDelta ? ` (${d.agentDelta > 0 ? "+" : ""}${d.agentDelta} agents)` : ""}`);
        lines.push("");
    }
    if (e.budget?.usd !== null && e.budget?.usd !== undefined) {
        const over = e.budget.overBy ?? 0;
    ...
    lines.push("|:--|:--|:--|--:|--:|");
    for (const n of e.nodes) {
        lines.push(`| \`${n.id}\` | ${KIND_LABEL[n.kind]} | \`${n.model}\` | ${n.widthAssumed ? "~" : ""}${n.calls.expected} | ${money(n.usd.expected)} |`);
    }
    lines.push("");
    lines.push("`~` = width or count assumed rather than read from the spec.");
    lines.push("");
    lines.push(`**Assumptions:** ${e.assumptions.profiles.worker.input / 1000}k in / ${e.assumptions.profiles.worker.output} out per fan-out call, ` +
    ...
    lines.push("</details>");
    lines.push("");
    lines.push(`<sub>Estimated by [preflight](https://unchained-labs.github.io/preflight/) · prices verified ${e.pricingVerified} · this is an estimate, not a quote</sub>`);
    return lines.join("\n");
}
export { money };
