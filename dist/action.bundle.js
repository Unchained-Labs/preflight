...
// src/pricing.ts
var PRICING_VERIFIED = "2026-06-24";
var PRICING = {
  "claude-fable-5": { input: 10, output: 50, context: 1e6, tier: "deep" },
  "claude-mythos-5": { input: 10, output: 50, context: 1e6, tier: "deep" },
  "claude-opus-5": { input: 5, output: 25, context: 1e6, tier: "deep" },
  "claude-opus-4-8": { input: 5, output: 25, context: 1e6, tier: "deep" },
  "claude-opus-4-7": { input: 5, output: 25, context: 1e6, tier: "deep" },
  "claude-opus-4-6": { input: 5, output: 25, context: 1e6, tier: "deep" },
  "claude-sonnet-5": {
  ...
  "claude-sonnet-4-6": { input: 3, output: 15, context: 1e6, tier: "standard" },
  "claude-haiku-4-5": { input: 1, output: 5, context: 2e5, tier: "cheap" }
};
var UNTIERED_ASSUMPTION = "claude-opus-5";
function priceOf(model, asOf) {
...
function resolveModel(model) {
  ...
  const exact = Object.keys(PRICING).sort((a, b) => b.length - a.length).find((k) => lower.includes(k));
...
function classify(node) {
  ...
  if (/^(scope|plan|decompose|enumerate|index|split)/.test(id)) return "scope";
  if (/(report|synth|write|summar|rank|adjudicat|digest)/.test(id)) return "synthesis";
  if (node.fanout !== null && node.fanout !== 1) return "worker";
  return node.terminal ? "synthesis" : "worker";
}
function usdFor(calls, profile, price, a) {
  if (calls <= 0) return { usd: 0, input: 0, output: 0 };
...
function estimate(spec, overrides = {}) {
  const a = {
    ...DEFAULT_ASSUMPTIONS,
  ...
  if (spec.hasCycle && !spec.cycleRounds) {
    warnings.push(
      `the graph has a cycle with no declared round cap \u2014 assuming ${rounds.low}/${rounds.expected}/${rounds.high} rounds. Declare a cap and this estimate gets much tighter.`
    );
  }
  for (const node of spec.nodes) {
    const kind = classify(node);
    if (kind === "verifier") continue;
    ...
    const width = node.fanout && node.fanout > 0 ? { low: node.fanout, expected: node.fanout, high: node.fanout } : widthAssumed ? a.unknownFanout : { low: 1, expected: 1, high: 1 };
    ...
    if (node.model === null && node.tier === null) {
      warnings.push(
        `node "${node.id}" sets no model or tier, so it inherits the session model. Priced as ${price.model}.`
      );
    }
  }
  for (const node of spec.nodes) {
    if (classify(node) !== "verifier") continue;
    ...
    const findings = declared ? { low: declared, expected: declared, high: declared } : {
    ...
    if (declared === null && workerCalls.expected === 0) {
      warnings.push(
        `node "${node.id}" is a verifier but nothing upstream produces findings and it declares no \`fanout.width\` \u2014 priced at zero. Declare a width if this stage runs standalone.`
      );
    }
    const calls = {
    ...
    nodes.push({
      ...
      note: declared ? `${lenses} lens${lenses === 1 ? "" : "es"} \xD7 ${declared} declared findings` : `${lenses} lens${lenses === 1 ? "" : "es"} \xD7 findings from ${workerCalls.expected} worker calls`
    });
    if (node.lenses.length > 1 && new Set(node.lenses).size < node.lenses.length) {
      warnings.push(
        `node "${node.id}" declares duplicate lenses \u2014 you are paying for them and getting one check. See decorrelate.`
      );
    }
  }
  ...
  const drivers = nodes.map((n) => ({
    ...
    lever: leverFor(n)
  })).filter((d) => d.usd > 0).sort((x, y) => y.usd - x.usd).slice(0, 5);
  const budget = spec.budget ? {
    usd: spec.budget.usd,
    tokens: spec.budget.tokens,
    overBy: spec.budget.usd !== null ? usd.expected - spec.budget.usd : null
  } : null;
  if (budget?.overBy !== null && budget?.overBy !== void 0 && budget.overBy > 0) {
    warnings.push(
      `the expected cost of $${usd.expected.toFixed(2)} exceeds the declared budget of $${budget.usd.toFixed(2)} by $${budget.overBy.toFixed(2)}.`
    );
  }
  return {
    name: spec.name,
...
function leverFor(n) {
  if (n.kind === "verifier") return "drop a lens, or replace one with an executable oracle";
  if (n.kind === "worker" && n.tier === "deep") return "this fan-out is on the deep tier \u2014 demote it";
  if (n.kind === "worker") return "narrow the fan-out, or add a zero-token prefilter";
  if (n.kind === "synthesis") return "usually correct to keep deep \u2014 judgment reaching the user";
  return "scope quality sets the ceiling downstream; usually worth the deep tier";
}
function diffEstimates(before, after) {
  if (!before) {
  ...
  for (const [id, n] of afterById) {
    ...
    if (!b) changes.push({ id, kind: "added", to: n.usd.expected });
    else if (Math.abs(b.usd.expected - n.usd.expected) > 5e-3) {
      changes.push({ id, kind: "changed", from: b.usd.expected, to: n.usd.expected });
    }
  }
  for (const [id, n] of beforeById) {
    if (!afterById.has(id)) changes.push({ id, kind: "removed", from: n.usd.expected });
  }
  return {
    before,
    after,
...
// src/report.ts
var colour = process.env.NO_COLOR === void 0 && Boolean(process.stdout.isTTY);
var c = (code) => (s) => colour ? `\x1B[${code}m${s}\x1B[0m` : s;
var bold = c("1");
var dim = c("2");
var grey = c("90");
var green = c("32");
...
var red = c("31");
var cyan = c("36");
var money = (n) => n < 1 ? `$${n.toFixed(3)}` : `$${n.toFixed(2)}`;
var compact = (n) => n >= 1e6 ? `${(n / 1e6).toFixed(1)}M` : n >= 1e3 ? `${Math.round(n / 1e3)}k` : String(Math.round(n));
var KIND_LABEL = {
  scope: "scope",
  worker: "fan-out",
  verifier: "verify",
...
var MARKER = "<!-- preflight-cost -->";
function markerFor() {
  return MARKER;
}
function markdown(d, opts = { file: "" }) {
  const e = d.after;
  const total = e.usd.expected || 1;
  const lines = [MARKER];
  const arrow = d.usdDelta === null ? "" : d.usdDelta > 5e-3 ? `\u{1F4C8} **+${money(d.usdDelta)}**` : d.usdDelta < -5e-3 ? `\u{1F4C9} **${money(d.usdDelta)}**` : "\u2192 no change";
  lines.push(`### Predicted cost \u2014 \`${e.name ?? opts.file}\``);
  lines.push("");
  lines.push(
    `| | agents | cost | tokens |
|:--|--:|--:|--:|
| **expected** | ${e.agents.expected} | **${money(e.usd.expected)}** | ${compact(e.tokens.input)} in \xB7 ${compact(e.tokens.output)} out |
| range | ${e.agents.low}\u2013${e.agents.high} | ${money(e.usd.low)}\u2013${money(e.usd.high)} | |`
  );
  lines.push("");
  if (arrow) {
    lines.push(
      `${arrow} against the base branch${d.agentDelta ? ` (${d.agentDelta > 0 ? "+" : ""}${d.agentDelta} agents)` : ""}`
    );
    lines.push("");
  }
  if (e.budget?.usd !== null && e.budget?.usd !== void 0) {
    const over = e.budget.overBy ?? 0;
    lines.push(
      over > 0 ? `> [!WARNING]
> Expected cost **exceeds the declared budget** of ${money(e.budget.usd)} by ${money(over)}.` : `\u2705 Within the declared budget of ${money(e.budget.usd)}.`
    );
    lines.push("");
  }
  ...
  if (e.byKind.verifier / total > 0.4) {
    lines.push(
      `> [!NOTE]
> Verification is ${Math.round(e.byKind.verifier / total * 100)}% of this run. Findings \xD7 lenses is usually the growth term, not fan-out width \u2014 dropping one lens is the cheapest lever here.`
    );
    lines.push("");
  }
  if (d.changes.length) {
    ...
    for (const ch of d.changes.slice(0, 15)) {
      const desc = ch.kind === "added" ? `added, ${money(ch.to)}` : ch.kind === "removed" ? `removed, was ${money(ch.from)}` : `${money(ch.from)} \u2192 ${money(ch.to)}`;
  ...
  lines.push("");
  lines.push(
    `<sub>Estimated by [preflight](https://unchained-labs.github.io/preflight/) \xB7 prices verified ${e.pricingVerified} \xB7 this is an estimate, not a quote</sub>`
  );
  return lines.join("\n");
}
...
function readSpec(source) {
  ...
  const nodes = rawNodes.filter((n) => typeof n?.id === "string").map((n) => {
    const lenses = Array.isArray(n.harness?.lenses) ? n.harness.lenses.filter((l) => typeof l === "string") : [];
    const width = typeof n.fanout?.width === "number" ? n.fanout.width : typeof n.fanout?.maxConcurrent === "number" ? n.fanout.maxConcurrent : n.fanout ? -1 : null;
    if (width === -1) {
      warnings.push(
        `node "${n.id}" fans out but states no width \u2014 using the assumed range. Add \`fanout.width\` for a tighter estimate.`
      );
    }
    return {
      id: n.id,
  ...
  const backEdges = edges.filter((e) => {
    ...
    return from !== void 0 && to !== void 0 && to <= from;
  });
  const hasCycle = backEdges.length > 0;
  if (hasCycle) {
    const inCycle = new Set(backEdges.flatMap((e) => [e.from, e.to]));
    for (const n of nodes) if (inCycle.has(n.id)) n.inCycle = true;
  }
...
var VERIFIER_WORDS = /verify|refute|skeptic|judge|adjudicate|is this real|critique/i;
function readScript(source) {
  ...
  let ast;
  try {
    ast = parse3(source, { ecmaVersion: "latest", sourceType: "module", locations: true });
  } catch (e) {
  ...
  const props = (n) => {
    ...
    if (n?.type !== "ObjectExpression") return m;
    for (const p of n.properties) {
      if (p.type === "Property" && !p.computed) {
        const k = p.key.type === "Identifier" ? p.key.name : p.key.value;
        if (typeof k === "string") m.set(k, p.value);
      }
    }
    return m;
  };
  const callee = (n) => {
    ...
    if (n.type === "MemberExpression") return `${callee(n.object)}.${n.property?.name ?? ""}`;
    return "";
  };
  ...
  const walk = (n, ctx) => {
    ...
    if (n.type === "CallExpression") {
      ...
      if (name2 === "parallel" || name2 === "pipeline") {
        const arg = n.arguments[0];
        let width = null;
        if (arg?.type === "ArrayExpression") width = arg.elements.length;
        const src = source.slice(n.start, n.end);
        const af = /Array\.from\(\s*\{\s*length\s*:\s*(\d+)/.exec(src);
        if (af) width = Number(af[1]);
        const inner = { ...ctx, width: width ?? -1 };
        for (const k of keys(n)) walkAny(n[k], inner);
        return;
      }
    }
    if (n.type === "WhileStatement" || n.type === "DoWhileStatement") {
      ...
      for (const k of keys(n)) walkAny(n[k], { ...ctx, inCycle: true });
      return;
    }
    for (const k of keys(n)) walkAny(n[k], ctx);
  };
  const walkAny = (v, ctx) => {
    if (Array.isArray(v)) for (const c2 of v) walk(c2, ctx);
    else walk(v, ctx);
  };
  walk(ast, { inCycle: false, width: null });
  if (nodes.length) nodes[nodes.length - 1].terminal = true;
  const lensArray = /(?:LENSES|lenses)\s*=\s*\[([^\]]*)\]/.exec(source);
  if (lensArray) {
    const count = lensArray[1].split(",").filter((s) => s.trim()).length;
    for (const node of nodes) if (node.isVerifier) node.lenses = Array.from({ length: count }, (_, i) => `lens${i}`);
  }
  if (hasCycle && !cycleRounds) {
    warnings.push("a cycle with no statically visible round cap \u2014 using the assumed range");
  }
  warnings.push(
    "this is a script, so fan-out widths and finding counts are mostly runtime values. Estimates from a declarative spec are far tighter."
  );
...
// src/action.ts
var env = (k, d = "") => process.env[k]?.trim() || d;
...
function findSpecs(patterns) {
  const out = /* @__PURE__ */ new Set();
  for (const p of patterns) {
    try {
      const files = execFileSync("git", ["ls-files", p], { encoding: "utf8" }).split("\n").map((s) => s.trim()).filter(Boolean);
...
async function upsertComment(body) {
  ...
  try {
    const existing = await (await fetch(`${api}?per_page=100`, { headers })).json();
    const mine = Array.isArray(existing) ? existing.find((c2) => (c2.body ?? "").includes(markerFor())) : void 0;
    const url = mine ? `https://api.github.com/repos/${repo}/issues/comments/${mine.id}` : api;
    ...
    log("warning", `could not post the comment: ${e.message}`);
  }
}
async function main() {
  const patterns = env("PREFLIGHT_PATHS", ".claude/workflows/**,**/*.graph.json,**/*.spec.json").split(/[\n,]/).map((s) => s.trim()).filter(Boolean);
  const configPath = env("PREFLIGHT_CONFIG", "preflight.json");
  const overrides = existsSync(configPath) ? JSON.parse(readFileSync(configPath, "utf8")) : {};
  ...
  for (const file of specs) {
    ...
    try {
      ...
      log("warning", `preflight: skipped ${file}: ${e.message}`);
      continue;
    }
