...
export interface SpecNode {
  ...
  tier: string | null;
  model: string | null;
  /** Fan-out width. `null` = single call, `-1` = fans out, width unknown. */
  fanout: number | null;
  lenses: string[];
  isVerifier: boolean;
  inCycle: boolean;
  /** No outbound edge — the last judgment in the graph. */
  terminal: boolean;
}
...
export interface CostSpec {
  ...
  nodes: SpecNode[];
  hasCycle: boolean;
  /** Declared round bounds, when the spec states a cap. */
  cycleRounds: { low: number; expected: number; high: number } | null;
...
export function readSpec(source: string): CostSpec {
  const raw = JSON.parse(source) as Record<string, any>;
  const warnings: string[] = [];

  const rawNodes: any[] = Array.isArray(raw.nodes) ? raw.nodes : [];
  const edges: any[] = Array.isArray(raw.edges) ? raw.edges : [];
  const hasOutbound = new Set(edges.map((e) => e.from));

  const nodes: SpecNode[] = rawNodes
    .filter((n) => typeof n?.id === "string")
    .map((n) => {
      const lenses: string[] = Array.isArray(n.harness?.lenses)
        ? n.harness.lenses.filter((l: unknown) => typeof l === "string")
        : [];
      const width =
        typeof n.fanout?.width === "number"
          ? n.fanout.width
  ...
  let cycleRounds: CostSpec["cycleRounds"] = null;
  const capped = rawNodes.find((n) => typeof n?.loop?.maxRounds === "number");
  if (capped) {
    const max = capped.loop.maxRounds as number;
 ...
 */
export function readScript(source: string): CostSpec {
  const warnings: string[] = [];
  const nodes: SpecNode[] = [];
  let hasCycle = false;
  let cycleRounds: CostSpec["cycleRounds"] = null;
  let name: string | null = null;
  let budget: CostSpec["budget"] = null;
  let seq = 0;
  ...
  try {
    ...
    throw new Error(`cannot parse script: ${(e as Error).message}`);
  }

  const text = (n: any): string | null => {
    if (!n) return null;
    if (n.type === "Literal" && typeof n.value === "string") return n.value;
    if (n.type === "TemplateLiteral") return n.quasis.map((q: any) => q.value.cooked ?? "").join(" ");
    return null;
  };
  const props = (n: any): Map<string, any> => {
  ...
  const walk = (n: any, ctx: { inCycle: boolean; width: number | null }): void => {
  ...
  const walkAny = (v: any, ctx: { inCycle: boolean; width: number | null }): void => {
    if (Array.isArray(v)) for (const c of v) walk(c, ctx);
    else walk(v, ctx);
  };
...
function keys(n: any): string[] {
  return Object.keys(n).filter((k) => !["type", "start", "end", "loc", "range"].includes(k));
}

export function read(file: string, source: string): CostSpec {
