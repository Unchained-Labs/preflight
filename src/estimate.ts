...
import type { CostSpec, SpecNode } from "./spec.js";

/** Tokens assumed per call, per kind of node. Overridable in `preflight.json`. */
export interface TokenProfile {
  input: number;
  output: number;
  /** Share of input served from the prompt cache on calls after the first. */
  cacheHitRate: number;
}

export type NodeKind = "scope" | "worker" | "verifier" | "synthesis";

export const DEFAULT_PROFILES: Record<NodeKind, TokenProfile> = {
...
export interface Assumptions {
  profiles: Record<NodeKind, TokenProfile>;
  /** Findings produced per worker node, for the verification term. */
  findingsPerWorker: { low: number; expected: number; high: number };
  /** Fan-out width when the spec does not state one. */
  unknownFanout: { low: number; expected: number; high: number };
  /** Cycle rounds when the spec does not state a cap. */
...
export const DEFAULT_ASSUMPTIONS: Omit<Assumptions, "asOf"> = {
...
export interface NodeEstimate {
  ...
  model: string;
  tier: string;
  /** Calls at the expected width. */
  calls: { low: number; expected: number; high: number };
  usd: { low: number; expected: number; high: number };
  inputTokens: number;
  outputTokens: number;
  /** Set when the width was assumed rather than read from the spec. */
...
export interface Estimate {
  ...
  nodes: NodeEstimate[];
  /** Cost by phase of the graph, so the expensive stage is obvious. */
  byKind: Record<NodeKind, number>;
  /** Ranked drivers: what to change to move the number most. */
  drivers: { what: string; usd: number; share: number; lever: string }[];
  /** Declared budget, if the spec states one, and whether we blow it. */
  budget: { usd: number | null; tokens: number | null; overBy: number | null } | null;
  warnings: string[];
...
export function classify(node: SpecNode): NodeKind {
...
export function estimate(spec: CostSpec, overrides: Partial<Assumptions> = {}): Estimate {
  const a: Assumptions = {
    ...DEFAULT_ASSUMPTIONS,
  ...
  const byKind: Record<NodeKind, number> = { scope: 0, worker: 0, verifier: 0, synthesis: 0 };
  ...
  const sum = (k: "low" | "expected" | "high") => nodes.reduce((s, n) => s + n.usd[k], 0);
  const agents = {
...
function leverFor(n: NodeEstimate): string {
...
/** The diff between two estimates, for the PR comment. */
export interface EstimateDiff {
  before: Estimate | null;
  after: Estimate;
  usdDelta: number | null;
  agentDelta: number | null;
  /** Nodes added, removed, or repriced. */
  changes: { id: string; kind: "added" | "removed" | "changed"; from?: number; to?: number }[];
}

export function diffEstimates(before: Estimate | null, after: Estimate): EstimateDiff {
  if (!before) {
  ...
  const changes: EstimateDiff["changes"] = [];
